...
public class PtrLayout extends ViewGroup {
    ...
    // Scrolling duration status refresh complete -> default, setRefreshing(false)
    private int mRefreshCompleteToDefaultScrollingDuration = DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION;
    // Scrolling duration status default -> refreshing, mainly for auto refresh, setRefreshing(true)
    ...
    // Scrolling duration status load more complete -> default, setLoadingMore(false)
    ...
    // Scrolling duration status default -> loading more, mainly for auto load more, setLoadingMore(true)
    ...
    public PtrLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PtrLayout, defStyleAttr, 0);
        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.PtrLayout_refresh_enabled) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.PtrLayout_load_more_enabled) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.PtrLayout_swipe_style) {
                    setPtrStyle(a.getInt(attr, STYLE.CLASSIC));

                } else if (attr == R.styleable.PtrLayout_drag_ratio) {
                    setDragRatio(a.getFloat(attr, DEFAULT_DRAG_RATIO));

                } else if (attr == R.styleable.PtrLayout_refresh_final_drag_offset) {
                    setRefreshFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PtrLayout_load_more_final_drag_offset) {
                    setLoadMoreFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PtrLayout_refresh_trigger_offset) {
                    setRefreshTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PtrLayout_load_more_trigger_offset) {
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.PtrLayout_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_release_to_refreshing_scrolling_duration) {
                    setReleaseToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_refresh_complete_delay_duration) {
                    setRefreshCompleteDelayDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.PtrLayout_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_default_to_refreshing_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_release_to_loading_more_scrolling_duration) {
                    setReleaseToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_load_more_complete_delay_duration) {
                    setLoadMoreCompleteDelayDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.PtrLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.PtrLayout_default_to_loading_more_scrolling_duration) {
    ...
    protected void onFinishInflate() {
        ...
        if (mHeaderView != null && mHeaderView instanceof PtrTrigger) {
            mHeaderView.setVisibility(GONE);
        }
        if (mFooterView != null && mFooterView instanceof PtrTrigger) {
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new PtrLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    }

    @Override
    protected ViewGroup.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new PtrLayout.LayoutParams(p);
    }

    @Override
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new PtrLayout.LayoutParams(getContext(), attrs);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                // take over the ACTION_MOVE event from PtrLayout#onInterceptTouchEvent()
    ...
    public void setPtrStyle(int style) {
        this.mStyle = style;
        requestLayout();
    }

    // set how hard to drag. bigger easier, smaller harder; DEFAULT_DRAG_RATIO
    public void setDragRatio(float dragRatio) {
        this.mDragRatio = dragRatio;
    }

    // If the offset you set is smaller than headerHeight, use HeaderHeight as default
    public void setRefreshTriggerOffset(int offset) {
        mRefreshTriggerOffset = offset;
    }

    // If the offset you set is smaller than mFooterHeight, use mFooterHeight as default
    public void setLoadMoreTriggerOffset(int offset) {
        mLoadMoreTriggerOffset = offset;
    }

    // Set the final offset you can pull to refresh.
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onPrepare() {
            if (mHeaderView != null && mHeaderView instanceof PtrTrigger && STATUS.isStatusDefault(mStatus)) {
                mHeaderView.setVisibility(VISIBLE);
                ((PtrTrigger)mHeaderView).onPrepare();
            }
        }

        @Override
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mHeaderView != null && mHeaderView instanceof PtrTrigger && STATUS.isRefreshStatus(mStatus)) {
                if (mHeaderView.getVisibility() != VISIBLE) {
                    mHeaderView.setVisibility(VISIBLE);
                }
                ((PtrTrigger)mHeaderView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mHeaderView != null && mHeaderView instanceof PtrTrigger && STATUS.isReleaseToRefresh(mStatus)) {
                ((PtrTrigger)mHeaderView).onRelease();
            }
        }
        ...
        public void onRefresh() {
            if (mHeaderView != null && STATUS.isRefreshing(mStatus)) {
                if (mHeaderView instanceof PtrRefreshTrigger) {
                    ((PtrRefreshTrigger)mHeaderView).onRefresh();
                }
        ...
        public void onComplete() {
            if (mHeaderView != null && mHeaderView instanceof PtrTrigger) {
                ((PtrTrigger)mHeaderView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mHeaderView != null && mHeaderView instanceof PtrTrigger && STATUS.isStatusDefault(mStatus)) {
                ((PtrTrigger)mHeaderView).onReset();
                mHeaderView.setVisibility(GONE);
            }
        }
    };
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        @Override
        public void onPrepare() {
            if (mFooterView != null && mFooterView instanceof PtrTrigger && STATUS.isStatusDefault(mStatus)) {
                mFooterView.setVisibility(VISIBLE);
                ((PtrTrigger)mFooterView).onPrepare();
            }
        }

        @Override
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mFooterView != null && mFooterView instanceof PtrTrigger && STATUS.isLoadMoreStatus(mStatus)) {
                if (mFooterView.getVisibility() != VISIBLE) {
                    mFooterView.setVisibility(VISIBLE);
                }
                ((PtrTrigger)mFooterView).onMove(y, isComplete, automatic);
            }
        }

        @Override
        public void onRelease() {
            if (mFooterView != null && mFooterView instanceof PtrTrigger && STATUS.isReleaseToLoadMore(mStatus)) {
                ((PtrTrigger)mFooterView).onRelease();
            }
        }
        ...
        public void onLoadMore() {
            if (mFooterView != null && STATUS.isLoadingMore(mStatus)) {
                if (mFooterView instanceof PtrLoadMoreTrigger) {
                    ((PtrLoadMoreTrigger)mFooterView).onLoadMore();
                }
        ...
        public void onComplete() {
            if (mFooterView != null && mFooterView instanceof PtrTrigger) {
                ((PtrTrigger)mFooterView).onComplete();
            }
        }
        ...
        public void onReset() {
            if (mFooterView != null && mFooterView instanceof PtrTrigger && STATUS.isStatusDefault(mStatus)) {
                ((PtrTrigger)mFooterView).onReset();
                mFooterView.setVisibility(GONE);
            }
        }
    };

    public interface PtrLoadMoreTrigger {
        void onLoadMore();
    }
    ...
    public interface PtrRefreshTrigger {
        void onRefresh();
    }
    ...
    abstract class RefreshCallback implements PtrTrigger, PtrRefreshTrigger {}

    abstract class LoadMoreCallback implements PtrTrigger, PtrLoadMoreTrigger {}
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                ...
                mmLastY = currY;
                PtrLayout.this.autoScroll(yDiff);
                post(this);
            }
        }
        ...
        // It's just like the yScrolled param in the updateScroll(float yScrolled)
